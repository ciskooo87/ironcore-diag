...
import { WORKFLOW_STEPS, type WorkflowStepKey } from "@/lib/diag-workflow";
...
export function DiagShell({
  ...
  subtitle?: string;
  children: ReactNode;
  project?: { name: string; code: string; client?: string; workflowState?: string };
  active?: string;
  score?: number;
  status?: string;
  cta?: ReactNode;
  ...
  return (
    <main className="min-h-screen bg-[#0F172A] text-slate-100">
      <div className="mx-auto flex max-w-[1600px] gap-4 px-3 py-4 md:px-5 lg:gap-5">
        <aside className="hidden w-[260px] shrink-0 rounded-3xl border border-slate-800 bg-[#111827] p-4 lg:block">
          <div className="mb-6">
            ...
            {NAV.map((item) => {
              ...
              return (
                <Link
                  key={item.key}
                  href={href}
                  className={`block rounded-2xl border px-3 py-3 text-sm transition ${isActive ? "border-cyan-400/30 bg-cyan-400/10 text-cyan-100" : "border-slate-800 bg-slate-950/20 text-slate-300 hover:border-slate-700 hover:bg-slate-900/50"}`}
                >
                  {item.label}
                </Link>
              );
          ...
          <header className="mb-4 rounded-3xl border border-slate-800 bg-[#111827] p-4 md:p-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                {project ? (
                  <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-300">
                    <span className="rounded-full border border-slate-700 px-3 py-1">Projeto: {project.name}</span>
                    <span className="rounded-full border border-slate-700 px-3 py-1">Cliente: {project.client || project.name}</span>
              ...
              <div className="flex flex-wrap items-center gap-3 xl:justify-end">
                <div className="rounded-2xl border border-slate-800 bg-slate-950/30 px-4 py-3 text-sm">
                  <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Status</div>
                  <div className="mt-1 font-medium text-white">{status}</div>
                </div>
                <div className={`rounded-2xl border px-4 py-3 text-center ${scoreTone(score)}`}>
                  <div className="text-xs uppercase tracking-[0.18em]">Score geral</div>
                  <div className="mt-1 text-2xl font-semibold">{score}</div>
                </div>
                {cta ? <div>{cta}</div> : null}
                <form action={appPath("/api/auth/logout/")} method="post">
                  <button type="submit" className="rounded-2xl border border-slate-700 bg-slate-950/30 px-4 py-3 text-sm text-slate-200 hover:border-slate-600">Sair</button>
                </form>
              </div>
            </div>
          </header>
