...
export function ProductHero({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children?: ReactNode }) {
  return (
    <section className="product-hero mb-4 rounded-[28px] border border-black/5 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          <div className="product-eyebrow inline-flex rounded-full border border-black/10 bg-[#F8FAFC] px-3 py-1 text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-[#667085]">{eyebrow}</div>
...
export function StatusPill({ label, tone = "neutral" }: { label: string; tone?: "neutral" | "good" | "warn" | "bad" | "info" }) {
  const toneClass = tone === "good" ? "border-[#ABEFC6] bg-[#ECFDF3] text-[#027A48]"
    : tone === "warn" ? "border-[#FEDF89] bg-[#FFFAEB] text-[#B54708]"
    : tone === "bad" ? "border-[#FECDCA] bg-[#FEF3F2] text-[#B42318]"
    : tone === "info" ? "border-black/10 bg-white text-[#0F172A]"
    : "border-black/5 bg-[#F8FAFC] text-[#475467]";
