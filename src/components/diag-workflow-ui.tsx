...
export function WorkflowChecklist({ items, compact = false }: { items: WorkflowItem[]; compact?: boolean; }) {
  return (
    <div className={`grid gap-3 ${compact ? "" : "md:grid-cols-2"}`}>
      {items.map((item) => (
        <div key={item.key} className={`rounded-2xl border p-4 ${item.done ? "border-[#ABEFC6] bg-[#ECFDF3] text-[#027A48]" : "border-black/5 bg-[#F8FAFC] text-[#475467]"}`}>
...
export function StepGuidance({ title, description, nextHref, nextLabel }: { title: string; description: string; nextHref?: string; nextLabel?: string; }) {
  return (
    <section className="rounded-3xl border border-black/5 bg-[#F8FAFC] p-5 md:p-6">
      ...
      {nextHref && nextLabel ? <div className="mt-4"><Link href={nextHref} className="inline-flex rounded-2xl bg-[#0F172A] px-4 py-3 text-sm font-medium text-white hover:bg-[#111827]">{nextLabel}</Link></div> : null}
    </section>
  );
}
