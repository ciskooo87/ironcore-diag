  ...
  return (
    <div className="space-y-3">
      <div className="min-w-0 rounded-2xl border border-black/5 bg-[#F8FAFC] p-4 md:p-5">
        ...
        <textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Pergunte algo sobre causa raiz, cenário, risco, dívida ou recomendação..." className="mt-3 min-h-28 w-full rounded-lg border border-black/5 bg-[#F8FAFC] px-3 py-2 text-sm leading-6 text-[#FAFAF7]" />
      ...
      <div className="grid gap-2 text-sm">
        {[
          ...
          <button key={item} type="button" onClick={() => ask(item)} className="rounded-xl border border-black/5 bg-[#F8FAFC] px-3 py-3 text-left leading-6 text-[#475467] hover:border-white/15 hover:text-[#101828]">{item}</button>
        ))}
      </div>
