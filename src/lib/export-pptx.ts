...
function addTitle(slide: PptxGenJS.Slide, title: string, subtitle?: string) {
  ...
  slide.addText(title, { x: 0.45, y: 0.18, w: 8.5, h: 0.3, fontSize: 24, bold: true, color: "FFFFFF" });
  if (subtitle) slide.addText(subtitle, { x: 0.45, y: 0.98, w: 10, h: 0.35, fontSize: 12, color: "475569" });
}
...
export async function buildExecutivePptx(input: { projectName: string; client?: string; score?: number; report?: Report }) {
  ...
  slide.addShape("roundRect", { x: 0.55, y: 0.75, w: 12.2, h: 5.85, fill: { color: "FFFFFF" }, line: { color: "E2E8F0" } });
  slide.addShape("rect", { x: 0.55, y: 0.75, w: 12.2, h: 1.05, fill: { color: "0F172A" }, line: { color: "0F172A" } });
  slide.addText("IRONCORE DIAG", { x: 0.9, y: 1.0, w: 2.5, h: 0.25, fontSize: 11, bold: true, color: "CBD5E1" });
  slide.addText("Diagnóstico Executivo Final", { x: 0.9, y: 1.42, w: 6.6, h: 0.45, fontSize: 26, bold: true, color: "0F172A" });
  slide.addText(`Cliente: ${input.client || input.projectName}\nProjeto: ${input.projectName}\nScore geral: ${input.score || "-"}`, { x: 0.9, y: 2.1, w: 4.9, h: 1.2, fontSize: 15, color: "334155" });
  slide.addShape("roundRect", { x: 7.8, y: 2.05, w: 4.0, h: 1.2, fill: { color: "EFF6FF" }, line: { color: "DBEAFE" } });
  slide.addText("Entrega executiva\npronta para decisão", { x: 8.1, y: 2.35, w: 3.4, h: 0.6, fontSize: 18, bold: true, color: "1D4ED8", align: "center" });
  slide.addText(String(report.executiveSummary || "-"), { x: 0.9, y: 3.7, w: 10.9, h: 2.0, fontSize: 18, color: "334155", margin: 0.04 });

  slide = pptx.addSlide();
  slide.background = { color: "FFFFFF" };
  addTitle(slide, "Resumo executivo", "Síntese do caso e principais indicadores");
  slide.addText(String(report.executiveSummary || "-"), { x: 0.5, y: 1.45, w: 12.1, h: 1.2, fontSize: 16, color: "334155", margin: 0.04 });
  (report.kpis || []).slice(0, 4).forEach((kpi, i) => {
    slide.addShape("roundRect", { x: 0.5 + i * 3.15, y: 3.05, w: 2.85, h: 1.3, fill: { color: "F8FAFC" }, line: { color: "E2E8F0" } });
    slide.addText(kpi.label, { x: 0.68 + i * 3.15, y: 3.27, w: 2.45, h: 0.25, fontSize: 10, color: "475569", bold: true, align: "center" });
    slide.addText(kpi.value, { x: 0.68 + i * 3.15, y: 3.65, w: 2.45, h: 0.35, fontSize: 18, color: "0F172A", bold: true, align: "center" });
  });
  slide.addShape("roundRect", { x: 0.5, y: 4.85, w: 5.95, h: 1.55, fill: { color: "F8FAFC" }, line: { color: "E2E8F0" } });
  slide.addShape("roundRect", { x: 6.75, y: 4.85, w: 5.95, h: 1.55, fill: { color: "0F172A" }, line: { color: "0F172A" } });
  slide.addText("Leitura do cenário", { x: 0.8, y: 5.12, w: 2.5, h: 0.25, fontSize: 14, bold: true, color: "0F172A" });
  slide.addText(String(report.scenarioReading || "-"), { x: 0.8, y: 5.45, w: 5.1, h: 0.75, fontSize: 12, color: "475569" });
  slide.addText("Direcionamento", { x: 7.05, y: 5.12, w: 2.5, h: 0.25, fontSize: 14, bold: true, color: "FFFFFF" });
  slide.addText((report.strategicDirection || []).slice(0, 2).map((x) => `• ${x}`).join("\n") || "-", { x: 7.05, y: 5.45, w: 5.1, h: 0.75, fontSize: 12, color: "E2E8F0" });
