...
export default async function DiagnosticoHistoricoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!project) return <DiagShell user={user} title="Validação IA" active="document"><div className="rounded-[28px] border border-black/5 bg-white p-5 surface-elevated text-sm text-[#B42318]">Projeto não encontrado.</div></DiagShell>;
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return <DiagShell user={user} title="Validação IA" active="document"><div className="rounded-[28px] border border-black/5 bg-white p-5 surface-elevated text-sm text-[#B42318]">Sem permissão.</div></DiagShell>;
    ...
    <DiagShell user={user} title="Validação da leitura IA" subtitle="Aqui a leitura da IA é revisada antes de virar entrega final. O objetivo é decidir com clareza: aprovar, ajustar ou bloquear." active="document" project={{ name: project.name, code: project.code, client: project.legal_name, workflowState: project.workflow_state }} score={presentation.overallScore} status={latest ? "Leitura IA disponível" : "Sem leitura IA ainda"} cta={<Link href={`/projetos/${id}/entrega-final/`} className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm font-medium text-[#0F172A] transition hover:-translate-y-0.5 hover:border-black/15 hover:bg-[#F8FAFC]">Ir para entrega final</Link>}>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="rounded-[28px] border border-black/5 bg-white p-6 surface-elevated md:p-7">
          <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#98A2B3]">Leitura IA</div>
          <h2 className="mt-2 text-[1.35rem] font-semibold tracking-[-0.03em] text-[#101828]">Resposta mais recente do motor</h2>
          <div className="mt-4 rounded-2xl border border-black/5 bg-[#F8FAFC] p-4 md:p-5 text-sm whitespace-pre-wrap text-[#475467]">
            {latest?.response || "Nenhuma resposta de IA registrada ainda."}
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 text-sm">
            <div className="rounded-2xl border border-black/5 bg-[#F8FAFC] p-4 md:p-5"><div className="text-xs uppercase tracking-[0.18em] text-[#98A2B3]">Provider</div><div className="mt-2 text-[#101828]">{latest?.provider || '-'}</div></div>
            <div className="rounded-2xl border border-black/5 bg-[#F8FAFC] p-4 md:p-5"><div className="text-xs uppercase tracking-[0.18em] text-[#98A2B3]">Modelo</div><div className="mt-2 text-[#101828]">{latest?.model || '-'}</div></div>
          </div>
        </section>
