...
function Field({ label, hint, name, defaultValue, placeholder, required = false, className = "", type = "text", suffix }: { label: string; hint?: string; name: string; defaultValue?: string | number; placeholder?: string; required?: boolean; className?: string; type?: string; suffix?: string }) {
  return (
    <label className={`block ${className}`}>
      <div className="mb-1 text-xs font-medium uppercase tracking-[0.14em] text-[#667085]">{label}</div>
      {hint ? <div className="mb-2 text-[11px] text-[#98A2B3]">{hint}</div> : null}
      <div className="relative">
        <input name={name} type={type} defaultValue={defaultValue} placeholder={placeholder} required={required} className="w-full rounded-lg border border-black/10 bg-white px-3 py-2 pr-10 text-sm" />
        {suffix ? <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[#98A2B3]">{suffix}</span> : null}
      </div>
    </label>
  );
}
...
function ParameterCard({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <div className="rounded-2xl border border-black/5 bg-[#F8FAFC] p-4"><div className="text-xs uppercase tracking-[0.18em] text-[#98A2B3]">{title}</div><div className="mt-1 text-xs text-[#98A2B3]">{description}</div><div className="mt-3 grid gap-3">{children}</div></div>;
}
...
export default async function CadastroPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  if (!project) return <DiagShell user={user} title="Cadastro" active="overview"><div className="rounded-3xl border border-black/5 bg-white p-5 text-sm text-[#B42318]">Projeto não encontrado.</div></DiagShell>;
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return <DiagShell user={user} title="Cadastro" active="overview"><div className="rounded-3xl border border-black/5 bg-white p-5 text-sm text-[#B42318]">Sem permissão.</div></DiagShell>;
  const presentation = await buildProjectPresentation(project);
  const fp = project.financial_profile || {};

  return <DiagShell user={user} title="Cadastro do projeto" subtitle="Confirme os dados-base do cliente e também os parâmetros financeiros usados na montagem sintética do DRE/DFC." active="overview" project={{ name: project.name, code: project.code, client: project.legal_name, workflowState: project.workflow_state }} score={presentation.overallScore} status={project.workflow_state || "cadastro"} cta={<Link href={`/projetos/${id}/upload-historico/`} className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm font-medium text-[#0F172A] transition hover:border-black/15 hover:bg-[#F8FAFC]">Próxima etapa</Link>}><div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]"><section className="rounded-[28px] border border-black/5 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] md:p-6"><div className="text-[11px] uppercase tracking-[0.24em] text-[#667085]">Cadastro</div><h2 className="mt-2 text-xl font-semibold text-[#101828]">Base do projeto + parâmetros DRE/DFC</h2><p className="mt-2 text-sm text-[#667085]">Além dos dados cadastrais, configure aqui os percentuais que alimentam DRE e DFC histórico/projetado.</p><form action={appPath(`/api/projects/${id}/cadastro/save/`)} method="post" className="mt-5 grid gap-3 text-sm md:grid-cols-2"><Field label="Nome do projeto" name="name" defaultValue={project.name} placeholder="Nome" required /><Field label="CNPJ" name="cnpj" defaultValue={project.cnpj} placeholder="CNPJ" required /><Field label="Razão social" name="legal_name" defaultValue={project.legal_name} placeholder="Razão social" required className="md:col-span-2" /><Field label="Segmento" name="segment" defaultValue={project.segment} placeholder="Segmento" required /><Field label="Sócios" name="partners" defaultValue={(project.partners || []).join(", ")} placeholder="Sócios" /><label className="block md:col-span-2"><div className="mb-1 text-xs font-medium uppercase tracking-[0.14em] text-[#667085]">Resumo do projeto</div><div className="mb-2 text-[11px] text-[#98A2B3]">Resumo executivo do caso, contexto e deterioração observada.</div><textarea name="project_summary" defaultValue={project.project_summary} placeholder="Resumo do projeto" className="min-h-32 w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm" /></label>

<div className="mt-2 rounded-2xl border border-black/5 bg-[#F8FAFC] p-4 md:col-span-2"><div className="text-xs uppercase tracking-[0.18em] text-[#98A2B3]">Parâmetros financeiros legados</div><div className="mt-1 text-xs text-[#98A2B3]">Mantidos por compatibilidade com regras antigas do projeto.</div><div className="mt-3 grid gap-3 md:grid-cols-4"><Field label="TX" hint="Campo legado" name="tx_percent" defaultValue={pct(fp.tx_percent, 0)} placeholder="0" suffix="%" /><Field label="Float" hint="Campo legado" name="float_days" defaultValue={pct(fp.float_days, 0)} placeholder="0" suffix="dias" /><Field label="TAC" hint="Campo legado" name="tac" defaultValue={pct(fp.tac, 0)} placeholder="0" suffix="R$" /><Field label="Custo por boleto" hint="Campo legado" name="cost_per_boleto" defaultValue={pct(fp.cost_per_boleto, 0)} placeholder="0" suffix="R$" /></div></div>
...
<button type="submit" className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-[#0F172A] hover:bg-[#F8FAFC] md:col-span-2">Salvar cadastro</button>{query.saved ? <div className="rounded-2xl border border-[#ABEFC6] bg-[#ECFDF3] px-4 py-3 text-sm text-[#027A48] md:col-span-2">Cadastro salvo. Siga para o upload das bases.</div> : null}{query.error ? <div className="rounded-2xl border border-[#FECDCA] bg-[#FEF3F2] px-4 py-3 text-sm text-[#B42318] md:col-span-2">Erro ao salvar parâmetros: {query.error}</div> : null}</form></section><StepGuidance title="Critério de saída" description="Agora o cadastro também define os parâmetros-base do DRE/DFC. Isso te permite calibrar a modelagem sintética por projeto em vez de depender de heurística fixa no código." nextHref={`/projetos/${id}/upload-historico/`} nextLabel="Ir para upload histórico" /></div></DiagShell>;
}
