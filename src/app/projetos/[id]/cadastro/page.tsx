...
function ParameterCard({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <div className="rounded-2xl border border-black/5 bg-[#F8FAFC] p-4"><div className="text-xs uppercase tracking-[0.18em] text-slate-500">{title}</div><div className="mt-1 text-xs text-slate-500">{description}</div><div className="mt-3 grid gap-3">{children}</div></div>;
}
...
export default async function CadastroPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  if (!project) return <DiagShell user={user} title="Cadastro" active="overview"><div className="rounded-3xl border border-black/5 bg-white p-5 text-sm text-rose-200">Projeto não encontrado.</div></DiagShell>;
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return <DiagShell user={user} title="Cadastro" active="overview"><div className="rounded-3xl border border-black/5 bg-white p-5 text-sm text-rose-200">Sem permissão.</div></DiagShell>;
...
<button type="submit" className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-[#0F172A] hover:bg-cyan-400/15 md:col-span-2">Salvar cadastro</button>{query.saved ? <div className="rounded-2xl border border-[#ABEFC6] bg-[#ECFDF3] px-4 py-3 text-sm text-[#027A48] md:col-span-2">Cadastro salvo. Siga para o upload das bases.</div> : null}{query.error ? <div className="rounded-2xl border border-[#FECDCA] bg-[#FEF3F2] px-4 py-3 text-sm text-[#B42318] md:col-span-2">Erro ao salvar parâmetros: {query.error}</div> : null}</form></section><StepGuidance title="Critério de saída" description="Agora o cadastro também define os parâmetros-base do DRE/DFC. Isso te permite calibrar a modelagem sintética por projeto em vez de depender de heurística fixa no código." nextHref={`/projetos/${id}/upload-historico/`} nextLabel="Ir para upload histórico" /></div></DiagShell>;
}
