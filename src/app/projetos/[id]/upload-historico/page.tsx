...
export default async function UploadHistoricoPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return (
    <AppShell user={user} title="Upload das bases históricas" subtitle="Etapa 3 do workflow">
      <ProductHero eyebrow="etapa 3" title="Suba as cinco bases obrigatórias" description="Faturamento, CAR, CAP, endividamento bancos e endividamento FIDC.">
        <StatusPill label={`Uploads: ${uploads.length}`} tone={uploads.length ? "good" : "warn"} />
        <Link href={`/projetos/${id}/contexto/`} className="pill">Próxima etapa</Link>
      </ProductHero>

      {query.saved ? <div className="alert ok-bg mb-4">Upload realizado.</div> : null}
      ...
      <section className="grid md:grid-cols-2 gap-3">
        {kinds.map(([kind, label]) => (
          <UploadHistoryForm
            key={kind}
            action={appPath(`/api/projects/${id}/daily/upload/`)}
            kind={kind}
            label={label}
            defaultDate={todayInSaoPauloISO()}
          />
        ))}
      </section>
    </AppShell>
