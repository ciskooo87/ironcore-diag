...
export default async function DiagnosticoPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  if (!project) return <DiagShell user={user} title="Diagnóstico IA" active="ia"><div className="rounded-[28px] border border-black/5 bg-white p-5 surface-elevated text-sm text-[#B42318]">Projeto não encontrado.</div></DiagShell>;
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return <DiagShell user={user} title="Diagnóstico IA" active="ia"><div className="rounded-[28px] border border-black/5 bg-white p-5 surface-elevated text-sm text-[#B42318]">Sem permissão.</div></DiagShell>;
  ...
  return (
    <DiagShell
      ...
      cta={<form action={appPath(`/api/projects/${id}/historical-diagnosis/run/`)} method="post"><button type="submit" className="rounded-2xl bg-[#0F172A] px-4 py-3 text-sm font-medium text-white shadow-[0_10px_24px_rgba(15,23,42,0.16)] hover:-translate-y-0.5 hover:bg-[#111827]">Rodar análise IA</button></form>}
    >
      <div className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <RightRail title="Prontidão para validação">
          ...
          <Link href={`/projetos/${project.code}/entrega-final/`} className="block rounded-2xl border border-black/5 bg-[#F8FAFC] px-4 py-3 text-sm transition hover:border-black/10 hover:bg-white text-[#344054] hover:border-black/10 hover:text-[#101828]">Ir para validação e entrega final</Link>
        </RightRail>
      </div>
    </DiagShell>
  );
