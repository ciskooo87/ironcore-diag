...
export default async function NormalizacaoPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  if (!project) return <DiagShell user={user} title="Normatização" active="inputs"><div className="rounded-[28px] border border-black/5 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] text-sm text-[#B42318]">Projeto não encontrado.</div></DiagShell>;
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return <DiagShell user={user} title="Normatização" active="inputs"><div className="rounded-[28px] border border-black/5 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] text-sm text-[#B42318]">Sem permissão.</div></DiagShell>;
    ...
    <DiagShell user={user} title="Normatização automática" subtitle="Aqui o sistema consolida cadastro, bases históricas e relato em uma estrutura única pronta para conferência e análise." active="inputs" project={{ name: project.name, code: project.code, client: project.legal_name, workflowState: project.workflow_state }} score={presentation.overallScore} status={project.workflow_state || "normalizacao"} cta={<Link href={`/projetos/${id}/conferencia/`} className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm font-medium text-[#0F172A] transition hover:border-black/15 hover:bg-[#F8FAFC]">Próxima etapa</Link>}>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="rounded-[28px] border border-black/5 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] md:p-6">
          ...
          <p className="mt-2 text-sm text-[#667085]">Esse passo prepara a leitura técnica do caso. A conferência vem logo depois para impedir que a IA rode em cima de estrutura mal fechada.</p>
          <form action={appPath(`/api/projects/${id}/normalization/run/`)} method="post" className="mt-5">
            <button type="submit" className="rounded-2xl bg-[#0F172A] px-4 py-3 text-sm font-medium text-white hover:bg-[#111827]">Rodar normatização</button>
          ...
          {project.ai_attention_points?.length ? <div className="mt-5 rounded-2xl border border-black/5 bg-[#F8FAFC] p-4 text-sm"><div className="font-medium text-[#101828]">Pontos de atenção extraídos</div><ul className="mt-3 space-y-2 text-[#475467]">{project.ai_attention_points.map((item) => <li key={item}>• {item}</li>)}</ul></div> : null}
        </section>
