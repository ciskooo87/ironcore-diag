...
export default async function NormalizacaoPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  if (!project) return <DiagShell user={user} title="Normatização" active="inputs"><div className="rounded-3xl border border-black/5 bg-white p-5 text-sm text-rose-200">Projeto não encontrado.</div></DiagShell>;
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return <DiagShell user={user} title="Normatização" active="inputs"><div className="rounded-3xl border border-black/5 bg-white p-5 text-sm text-rose-200">Sem permissão.</div></DiagShell>;
  const presentation = await buildProjectPresentation(project);
  const workflow = await buildWorkflowChecklist(project);

  return (
    <DiagShell user={user} title="Normatização automática" subtitle="Aqui o sistema consolida cadastro, bases históricas e relato em uma estrutura única pronta para conferência e análise." active="inputs" project={{ name: project.name, code: project.code, client: project.legal_name, workflowState: project.workflow_state }} score={presentation.overallScore} status={project.workflow_state || "normalizacao"} cta={<Link href={`/projetos/${id}/conferencia/`} className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-[#0F172A] hover:bg-cyan-400/15">Próxima etapa</Link>}>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="rounded-3xl border border-black/5 bg-white p-5 md:p-6">
          <div className="text-[11px] uppercase tracking-[0.24em] text-[#667085]">Normatização</div>
          <h2 className="mt-2 text-xl font-semibold text-[#101828]">Gerar estrutura consolidada</h2>
          ...
          <form action={appPath(`/api/projects/${id}/normalization/run/`)} method="post" className="mt-5">
            <button type="submit" className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-[#0F172A] hover:bg-cyan-400/15">Rodar normatização</button>
          ...
          {project.ai_attention_points?.length ? <div className="mt-5 rounded-2xl border border-slate-800 bg-slate-950/30 p-4 text-sm"><div className="font-medium text-[#101828]">Pontos de atenção extraídos</div><ul className="mt-3 space-y-2 text-slate-300">{project.ai_attention_points.map((item) => <li key={item}>• {item}</li>)}</ul></div> : null}
        </section>
