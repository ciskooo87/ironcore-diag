...
export default async function EntregaFinalPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return (
    <DiagShell user={user} title="Validação humana e entrega final" subtitle="Fechamento do diagnóstico com leitura executiva, demonstrativos financeiros, gráficos, decisão humana auditável e documento exportável." active="document" project={{ name: project.name, code: project.code, client: project.legal_name, workflowState: project.workflow_state }} score={presentation.overallScore} status={workflow.readyForFinalDelivery ? "Pronto para entrega final" : "Aguardando validação humana ou consolidação final"} cta={<div className="flex flex-wrap gap-2"><PrintButton /><Link href={`/api/projects/${id}/pdf/`} className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 px-4 py-3 text-sm text-cyan-100 hover:bg-cyan-400/15">Relatório executivo</Link><Link href={`/api/projects/${id}/xlsx/`} className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-100 hover:bg-emerald-400/15">Planilha analítica (.xlsx)</Link><Link href={`/api/projects/${id}/docx/`} className="rounded-2xl border border-violet-400/30 bg-violet-400/10 px-4 py-3 text-sm text-violet-100 hover:bg-violet-400/15">Resumo executivo (.docx)</Link><Link href={`/api/projects/${id}/pptx/`} className="rounded-2xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-100 hover:bg-amber-400/15">Apresentação (.pptx)</Link></div>}>
