...
export async function POST(req: Request, ctx: { params: Promise<{ code: string }> }) {
  ...
  if (!user || !project) return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?error=forbidden`));
  const allowed = await canAccessProject(user, project.id);
  if (!allowed) return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?error=forbidden`));
  ...
  if (Number.isNaN(daysAgo) || daysAgo < 0 || daysAgo > 3650) return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?error=date_limit`));
  if (!allowedKinds.has(uploadKind)) return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?error=upload_kind`));
  if (!(file instanceof File) || file.size === 0) return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?error=file_required`));
  ...
  try {
    ...
    await dbQuery(
      ...
      [project.id, dbUser?.id || null, "historical.upload", "daily_entries", id || null, JSON.stringify({ uploadKind, file: file.name, payload })]
    );
    return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?saved=1`));
  } catch {
    return NextResponse.redirect(publicUrl(req, `/projetos/${code}/upload-historico/?error=upload_parse`));
  }
}
