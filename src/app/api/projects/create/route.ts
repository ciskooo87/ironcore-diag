...
import { projectCodeFromName } from "@/lib/slug";
...
export async function POST(req: Request) {
  ...
  if (!name || !cnpj || !legalName || !segment) return NextResponse.redirect(publicUrl(req, "/dashboard?error=required"));

  const code = projectCodeFromName(name);
  const created = await createProject({ code, name, cnpj, legalName, segment, partners, timezone: "America/Sao_Paulo", accountPlan: [], projectSummary });
  const dbUser = await getUserByEmail(user.email);
  if (dbUser?.id && created?.id) {
    await dbQuery("insert into project_permissions(user_id, project_id, can_edit) values($1,$2,true) on conflict do nothing", [dbUser.id, created.id]).catch(() => null);
