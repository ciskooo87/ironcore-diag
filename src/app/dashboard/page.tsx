  ...
  return (
    <AppShell user={user} title="Dashboard" subtitle="Produto completo do diagnóstico: cadastro, upload, relato, normatização, IA, validação e entrega final">
      <ProductHero eyebrow="workflow completo" title="Diagnóstico histórico como produto de ponta a ponta" description="Fluxo: novo projeto > cadastro > upload bases > relato > normatização > conferência > diagnóstico > IA > validação humana > entrega final.">
        ...
        <Link href={appPath("/projetos/novo/")} className="pill">Novo projeto</Link>
      </ProductHero>
        ...
        {projects.map((project) => (
          <div key={project.id} className="card">
            ...
            <div className="flex gap-2 mt-4 flex-wrap">
              <Link href={appPath(`/projetos/${project.code}/cadastro/`)} className="pill">Cadastro</Link>
              <Link href={appPath(`/projetos/${project.code}/upload-historico/`)} className="pill">Upload</Link>
              <Link href={appPath(`/projetos/${project.code}/contexto/`)} className="pill">Relato</Link>
              <Link href={appPath(`/projetos/${project.code}/entrega-final/`)} className="pill">Entrega final</Link>
            </div>
          </div>
        ))}
      </div>
